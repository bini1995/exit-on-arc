...
const SEL = {
  ...
  liquidity: '0x1a686502', slot0: '0x3850c7bd', decimals: '0x313ce567', balanceOf: '0x70a08231'
};

function assert(condition, message) { if (!condition) throw new Error(message); }
const sleep = (ms) => new Promise((resolve) => setTimeout(resolve, ms));

async function json(url, options = {}, retries = 3) {
  let lastError;
  for (let attempt = 1; attempt <= retries; attempt++) {
    try {
      ...
      if (!transient || attempt === retries) throw new Error(`${url} -> HTTP ${response.status}`);
...
function addr(hex) { return `0x${String(hex).slice(-40)}`; }
function uint(hex) { return BigInt(hex || '0x0'); }
function balanceOf(owner) { return SEL.balanceOf + owner.toLowerCase().replace(/^0x/, '').padStart(64, '0'); }
...
function unit(raw, dec) { return Number(raw.toString()) / 10 ** dec; }
function words(hex) {
...
function money(n) { return Number(n).toLocaleString(undefined, { style: 'currency', currency: 'USD', maximumFractionDigits: 6 }); }

async function discoverCandidates() {
  const data = await json(`${ARCPAD}/api/tokens?limit=200`);
  const rows = Array.isArray(data?.creations) ? data.creations : Array.isArray(data?.tokens) ? data.tokens : Array.isArray(data) ? data : [];
...
async function testToken(seed) {
  ...
  if (arcPairs.length) console.log(`Top reported market: ${arcPairs[0].pairAddress} | liquidity=${money(arcPairs[0]?.liquidity?.usd || 0)}`);
  ...
  const token0 = addr(t0Hex), token1 = addr(t1Hex), lower = token.toLowerCase();
  ...
  const fee = Number(uint(feeHex)), L = uint(liqHex);
  assert(fee > 0, 'fee() returned 0'); assert(L > 0n, 'liquidity() returned 0');
  const slotWords = words(slotHex), sqrtP = BigInt(`0x${slotWords[0]}`);
  ...
  const b0 = unit(uint(b0Hex), dec0), b1 = unit(uint(b1Hex), dec1);
  const trackedIs0 = token0.toLowerCase() === lower, usdcIs0 = token0.toLowerCase() === USDC.toLowerCase();
  const usdcReserve = usdcIs0 ? b0 : b1, trackedBalance = trackedIs0 ? b0 : b1;
  ...
  const virtualIn = trackedIs0 ? xVirtual : yVirtual, virtualOut = trackedIs0 ? yVirtual : xVirtual;
  const feeFraction = fee / 1_000_000, sampleIn = Math.max(1, trackedBalance * 0.001);
  ...
  return { name: seed.name, symbol: seed.symbol, token, pool, dexPairs: arcPairs.length, launchPoolIndexed: Boolean(launchPair), usdcReserve };
}
...
for (const candidate of candidates) {
  results.push(await testToken(candidate));
  await sleep(500);
}
console.log('\n=== SUMMARY ===');
for (const result of results) console.log(`PASS ${result.name} (${result.symbol}) | Dex pairs=${result.dexPairs} | launchPoolIndexed=${result.launchPoolIndexed} | USDC=${money(result.usdcReserve)}`);
